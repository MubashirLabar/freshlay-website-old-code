...
import {confirmCode,resetandverifywithphone,sendCodeForVerifyEmail} from '../actions/auth'; 
// Screen
...
function Confirmcode(props) { 
    ...
    useEffect(() => {
        let themail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      //  console.log((emailnumber.match(themail)));
        if(emailnumber == null){ 
    ...
    const verifyCode = async () => {
        ...
        if(number){
        setloading(true)
        await dispatch(confirmCode(code,emailnumber,false,{number: true},{email : false}))
        setloading(false)
        }
        else if(email)
        {
        setloading(true)
        await dispatch(confirmCode(code,emailnumber,false,{number: false},{email : true}))
        setloading(false)
        }
    }
    const sendcodeagain = async () => {
        // if we dont send type it wont redirect
        if(number){
        setloading(true)
        ...
        {
        // sending true to notreload
        setloading(true)
        await dispatch(sendCodeForVerifyEmail(emailnumber,true)) 
        setloading(false)
        }
    }
        ...
        <React.Fragment>  
            ...
            <div className="signup-p rel">
                ...
                <div className="wrap recover-wrap abs abc"> 
                    ...
                    <div className="verify-blk flex flex-col">
                        <div className="title rfont b6 black">Verify {email ? 'email' : 'number'}</div>
                        <div className="sub font s15 b3 black">
                            Please enter the code first. which have been sended to your {email ? 'email' : 'number'}.
                        ...
                        <button 
                            className="button font s15 b6 anim" 
                            onClick={() => {
                                setcode(`${digit1}${digit2}${digit3}${digit4}${digit5}${digit6}`)
                                verifyCode(code)
                                
                            }}>
                            Verify {email ? 'email' : 'number'}
                        </button> 
                        <div className="txt font s15 c333">
                            Don't get code?&nbsp;
                            <button className="again color cleanbtn rfont s15 anim"
                                onClick={() => sendcodeagain(emailnumber)}
                            >Send again</button> 
                        </div>
                        </div>
                    </div> 
